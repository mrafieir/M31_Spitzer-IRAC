...
hits_ud = mrdfits("~/Projects/Andromeda/maps/I1_M31.NS.noCRs_mosaic_cov.fits", 0, header)
; replace all bad hits with 0
nans_ind = where(finite(hits_ud, /nan) or hits_ud le 0)
hits_ud[nans_ind] = 0d

; left-right hit map (i.e. along minor axis)
hits_lr = mrdfits("~/Projects/Andromeda/maps/I1_M31.EW.big_mosaic_cov.fits")
; replace all bad hits with 0
nans_ind = where(finite(hits_lr, /nan) or hits_lr le 0)
hits_lr[nans_ind] = 0d

; read catalogs (selected from the full catalog using TOPCAT)
up = mrdfits("~/Projects/Andromeda/cats/iracfits_u", 1) 
down = mrdfits("~/Projects/Andromeda/cats/iracfits_d", 1)
left = mrdfits("~/Projects/Andromeda/cats/iracfits_l", 1)
right = mrdfits("~/Projects/Andromeda/cats/iracfits_r", 1)
